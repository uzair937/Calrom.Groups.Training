...
namespace Overrides
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            finally
            {
                ...
                Console.WriteLine("");
                Console.WriteLine("Please enter two values by pressing enter between each of them.");
                ...
                try
                {
                    using (StreamWriter streamWriter = new StreamWriter("C:/Users/ufoolat/Downloads/comparison.txt"))
                    {
                        ...
                        JourneyComparer comparer = new JourneyComparer();
                        comparer = new FlightComparer();
                        comparer = new SectorComparer();
                        Console.WriteLine("Is the journey the same? " + comparer.Compare(secList[arg1], secList[arg2]));
                        Console.WriteLine("Journey one: " + secList[arg1].departureID + " " + secList[arg1].arrivalID + " || " +
                            "Journey two: " + secList[arg2].departureID + " " + secList[arg2].arrivalID);
                        Console.WriteLine("Is the flight the same? " + comparer.Compare(secList[arg1], secList[arg2]));
                        Console.WriteLine("Flight one: " + secList[arg1].flightNumber + " " + "Flight two: " + secList[arg2].flightNumber);
                        Console.WriteLine("Are the sectors the same? " + comparer.Compare(secList[arg1], secList[arg2]));
                        Console.WriteLine("Sector one: " + secList[arg1].sectorID + " " + "Sector two: " + secList[arg2].sectorID);
                    }
                }
                catch (Exception e)
                {
    ...
    class FlightComparer : JourneyComparer
    {
        public bool Compare(Flight flight1, Flight flight2)
        {
            return flight1.flightNumber == flight2.flightNumber && Compare((Journey)flight1, (Journey)flight2);
        }
    }

    class SectorComparer : FlightComparer
    {
        public bool Compare(Sector sector1, Sector sector2)
        {
            return sector1.sectorID == sector2.sectorID && Compare((Flight)sector1, (Flight)sector2);
        }
    }
}