...
namespace Calrom.Training.SocialMedia.Database.ORMRepositories
{
    public class UserRepository : IRepository<UserModel>
    {
        ...
        private List<UserModel> CleanseReturn(List<UserModel> users)
        {
            var newUsers = new List<UserModel>();
            foreach (var user in users)
            {
                newUsers.Add(CleanseReturn(user));
            }
            return newUsers;
        }
        ...
        public IEnumerable<UserModel> List()
        {
            ...
            using (var session = NHibernateHelper.OpenSession())
            {
                ...
                newList.Concat(CleanseReturn(userList));
            }
            return newList;
        }
        ...
        public IEnumerable<UserModel> GetFollowedUsers(int userId)
        {
            ...
            using (var session = NHibernateHelper.OpenSession())
            {
                ...
                newUsers.Concat(CleanseReturn(session.Query<UserModel>().Where(x => userModel.Following.Select(f => f.FollowingId).Contains(x.UserId)).ToList()));
